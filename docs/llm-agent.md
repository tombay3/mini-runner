...
- `retreat_from_guard`: move or climb away from medium/high/critical same-row guard danger in non-god mode.
...
- In normal mode, horizontal candidates toward a same-row guard are suppressed at medium, high, or critical risk. Horizontal actions that remain under guard pressure are capped at four ticks so risk is reassessed quickly; dig and vertical escape durations are unchanged.
